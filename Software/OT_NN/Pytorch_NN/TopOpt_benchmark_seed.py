...
# Seed-noise assessment for a single, fixed configuration.
#
# Trains N_SEEDS identical U-Net models that differ ONLY by their random seed,
# then runs the hybrid topology-optimization benchmark for each of them and
# stores every per-sample result (tagged with its seed) in benchmark_seed.csv.
# Aggregating the per-seed means finally gives the seed-to-seed variability,
# i.e. the reference error bar below which two configurations of the main
# architecture benchmark cannot be told apart.
#
...
RESET_BENCHMARK = True # overwrite benchmark_seed.csv (False -> append)


def seed_tag(seed):
    """Usual model tag for this configuration, suffixed with the seed."""
...
#%% Phase 1 — train one model per seed
...
if RUN_TRAINING:
    ...
    for SEED in SEEDS:
        print("\n" + "=" * 79)
        print(f"TRAIN  seed={SEED}  |  {NETWORK} NIF={NIF} N_conv={N_CONV} "
        ...
        # Augmentation applies to the train split only (disabled for this config).
...
""", nargout=0)
    ...
    # ── Progress window over every (seed, sample) pair ──
    total  = len(SEEDS) * SIZE_LOOP
...
#%% Phase 3 — read results & quantify the seed-to-seed noise
...
print("=" * 60)
for label, col in [('FEM-iteration ratio', 'mean_ratio'),
