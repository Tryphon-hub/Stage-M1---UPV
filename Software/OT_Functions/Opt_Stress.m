function [c,dc,ce,InfVol] = Opt_Stress(k,Stress,D,Penal,MeshData,ElemTO,SquareElm,Dim,NFpE)

%Esta función devuelve la derivada de la energía de deformación en cada
%elemento, a partir de las tensiones y las propiedades de material
%de cada elemento
...
for i = 1:LoopLength
    ...
    for iElem = Elements'
        ...
        for iGP = 1:NGP
            ...
            dc(i) = dc(iElem) - derDensity * 1/(k(iElem)^2*1) * Stress(iElem,IndexStress) * D^(-1) * Stress(iElem,IndexStress)' * Jacobian(iGP) * Wg(iGP);
            ce(iElem) = ce(iElem) + Density * 1/(k(iElem)^2*1) * Stress(iElem,IndexStress) * D^(-1) * Stress(iElem,IndexStress)' * Jacobian(iGP) * Wg(iGP);
